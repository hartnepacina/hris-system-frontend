...
const EmployeeList = () => {
  ...
  const openEdit = (emp: Employee) => {
    ...
    const [last, first] = emp.name.split(',');
    setFormData({
        ...
        <div className="overflow-x-auto">
          <table className="pro-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-400">No employees found matching your criteria.</td>
                </tr>
              ) : (
                filtered.map((emp) => (
                  <tr key={emp.id} className="hover:no-underline hover:bg-white">
                    ...
                    <td>
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => openView(emp)} className="btn-ghost btn-icon text-blue-500" title="View">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button onClick={() => openEdit(emp)} className="btn-ghost btn-icon text-emerald-600" title="Edit">
      ...
      {showAddModal && (
        <div className="pro-modal-overlay">
          <div className="pro-modal max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pro-modal-body">{renderFormFields(handleAdd, 'Add Employee')}</div>
          </div>
        </div>
      )}
      ...
      {showEditModal && (
        <div className="pro-modal-overlay">
          <div className="pro-modal max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pro-modal-body">{renderFormFields(handleEdit, 'Save Changes')}</div>
          </div>
        </div>
      )}
